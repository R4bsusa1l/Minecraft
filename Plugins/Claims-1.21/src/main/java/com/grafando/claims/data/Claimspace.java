package main.java.com.grafando.claims.data;
...
public class Claimspace {
    ...
    String SqlSelectAllClaims = "Select Block1_World, Block1_X, Block1_Z, Claimname from Claims where ClaimownerUUID = ?;";
    String SqlUpdateClaimName = "Update Claims set Claimname = ? where ClaimId = ?;";
    ...
    public ResultSet getAllClaimsByUuid(Player player, Connection conn){
        try{
            ps = conn.prepareStatement(SqlSelectAllClaims);
    ...
    public void updateClaimName(int claimId, String claimName, Connection conn){
        try{
            ps = conn.prepareStatement(SQLIncreaseActivityIndicator);
